...
収録15作品: 晩鐘（ミレー）／叫び（ムンク）／真珠の耳飾りの少女（フェルメール）／
神奈川沖浪裏（北斎）／我が子を食らうサトゥルヌス（ゴヤ）／イカロスの墜落のある
風景（ブリューゲル）／オフィーリア（ミレイ）／メキシコ湾流（ウィンズロー・
ホーマー）／夢魔（フューズリ）／民衆を導く自由の女神（ドラクロワ）／
アメリカン・ゴシック（グラント・ウッド）／ラス・メニーナス（ベラスケス）／
灰色と黒のアレンジメント第1番（ホイッスラー）／夜警（レンブラント）／
グランド・ジャット島の日曜日の午後（スーラ）

### 3.2 実行時の回答状態（JS変数、localStorageに永続化）

```js
answers = { [workId]: string }  // 回答済みの作品だけキーが存在する。localStorage["answers"]にJSONで保存
shareOptIn = boolean            // 「みんなの回答に公開する」チェックの状態（localStorageに保存）
```

localStorageのキー:
- `answers` — 上記の回答マップ（JSON）。ブラウザを閉じて再訪しても引き継がれる
- `shareOptIn` — オプトインの選択状態
- `liked_<Firestore docId>` — いいね済みフラグ（二重押下防止）
- `mine_<Firestore docId>` — このブラウザが投稿した回答のドキュメントID（「みんなの回答」で
  自分の投稿に「あなた」バッジを表示するために使用）

いずれもブラウザ単位のローカル保存で、ログイン機能は無い。別ブラウザ・別端末をまたぐと
引き継がれない。
...
- **進捗の永続化**: 回答は`localStorage`に保存され、ページを閉じて再訪しても回答済み／
  未回答の状態が引き継がれる（ログイン不要）
...
- **自分の回答バッジ**: 投稿時にFirestoreが返すドキュメントIDを`localStorage`に記録し、
  「みんなの回答」一覧で自分が投稿したものに「あなた」バッジを表示（ログイン・
  イニシャル入力は不要、ブラウザ単位）
...
- **結果画面のシェア行**: 𝕏（Twitter intent）／LINE（`line.me/R/msg/text/`）／
  リンクをコピー、の3導線
- **画像でシェアする**（答え合わせ画面）: Canvas APIで「絵＋回答」を
  ...
  答え合わせ画面上部の「画像でシェアする」は自分の回答、「みんなの回答」の
  各項目にある「画像でシェア」はその回答（他ユーザーのものも含む）を対象にする
  （同じ生成ロジックを共有）
...
- 進捗（`answers`）・自分の回答バッジ（`mine_*`）・いいね済みフラグ（`liked_*`）は
  すべてブラウザ単位の`localStorage`。別ブラウザ・別端末では引き継がれず、
  サイトのデータを消去すると失われる（アカウント機能はない）
